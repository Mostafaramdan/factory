...
class returnedOrders extends dashboard
{
    ...
    {
        $records= $this->model::where('order_type','returned');
    ...
    {
        $total_sold =$this->model::where('clients_id',$request->clients_id)
                        ->where('production_type_id',$request->production_type_id)
                        ->where('order_type','sold')
                        ->sum('quantity') -
                    $this->model::where('clients_id',$request->clients_id)
                        ->where('production_type_id',$request->production_type_id)
                        ->where('order_type','returned')
                        ->sum('quantity');

        if($total_sold < $request->quantity){
            return response()->json(['status'=>416,'total_sold'=>$total_sold]);
        }
        $client =clients::find($request->clients_id);
        $client->decrement('debits',$request->price * $request->quantity);
        productions::first()->increment('store',$request->quantity);
    ...
    public function destroy($id)
    {
        $record =$this->model::find($id);
        $client =clients::find($record->clients_id);
        $client->increment('debits',$record->price * $record->quantity);
        productions::first()->decrement('store',$record->quantity);
