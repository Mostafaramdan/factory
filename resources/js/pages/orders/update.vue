    ...
    <div class="m-3" >
        <form  @submit.prevent="onSubmit" class="border border-5 border-primary rounded form">
            ...
            <div class="row">
                ...
                <div class="col-6">
                    <label >  الارباح</label>
                    <input class="form-control" readonly :value='profits'>
                </div>
            </div>
            <hr>
            <dropdown-menu
...
<script>
  ...
  export default {
    ...
    methods: {
        async onSubmit() {
            this.loading=true;
            let response = await this.Api('PUT',`orders/${this.record.id}`,this.record);
            this.loading=false;
            if(response.data.status==200)
                this.$swal("تم التعديل بنجاح", "", "success")
            if(response.data.status==416)
                this.$swal(`الكمية المطلوبة غير متوفرة, الكمية المتوفرة = ${response.data.quantity}`, "", "error")
        },
    },
    computed: {
        validateQuantity(){
        ...
        profits(){
            if(this.averagePrice){
                ...
                this.record.profits= parseFloat(this.record.quantity) * profits_per_one;
    ...
    async mounted(){
        ...
        let response1 = await this.Api('GET','getProductionPrice',{});
        this.costs= response1.data;
    },
    metaInfo() {
        return {
