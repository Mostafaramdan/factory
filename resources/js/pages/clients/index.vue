...
export default {
    ...
    methods:{
        show(index){
             this.$router.push( {name:'clientsShow' , params: { id: this.records[index].id }});
        },
        create(){
             this.$router.push( {name:'clientsCreate'});
        },
        update(index){
             this.$router.push( {name:'clientsUpdate', params: { id: this.records[index].id }});
        },
        ...
        async getRecords(){
            ...
            let response = await this.Api('GET','clients',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`clients/${this.records[index].id}`);
    ...
    metaInfo() {
        return {
            title: `${this.$store.state.appName} -   العملاء  `,
        }
    },
    watch :{
        search:function(val){
