...
export default {
    ...
    methods:{
        create(){
             this.$router.push( {name:'returnedOrdersCreate',query:{...this.$route.query} });
        },
        update(index){
             this.$router.push( {name:'returnedOrdersUpdate', params: { id: this.records[index].id }});
        },
        show(index){
             this.$router.push( {name:'returnedOrdersShow' , params: { id: this.records[index].id }});
        },
        ...
        async getRecords(){
            ...
            let response = await this.Api('GET','returnedOrders',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`returnedOrders/${this.records[index].id}`);
    ...
    metaInfo() {
        return {
            title: `${this.$store.state.appName} -   المرتجع `,
        }
    },
    watch :{
        search:function(val){
