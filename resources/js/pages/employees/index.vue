            ...
            <table class="table table-striped table-dark table-bordered table-hover  mb-2"  >
                ...
                <tbody>
                    <tr v-for="(record,index) in records" :key="index">
                        ...
                        <td>{{record.name}}</td>
                        <td>{{record.phone}}</td>
                        <td>{{record.salary_type== 'monthly' ?  record.salary : 'لا يوجد'}}</td>
                        <td>{{record.salary_per_day}}</td>
...
export default {
    ...
    methods:{
        show(index){
             this.$router.push( {name:'employeesShow' , params: { id: this.records[index].id }});
        },
        create(){
             this.$router.push( {name:'employeesCreate'});
        },
        update(index){
             this.$router.push( {name:'employeesUpdate', params: { id: this.records[index].id }});
        },
        ...
        async getRecords(){
            ...
            let response = await this.Api('GET','employees',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`employees/${this.records[index].id}`);
    ...
    metaInfo() {
        return {
            title: `${this.$store.state.appName} -   الموظفين  `,
        }
    },
    watch :{
        search:function(val){
