<template>
    ...
    <form @submit.prevent="onSubmit" class="border border-5 border-primary rounded ">
        ...
        <div class="form-check ">
            ...
            <input type="text" v-model="record.salary" :class="['form-control' ,{'is-valid':validateSalary },{'is-invalid':!validateSalary}]"  >
            <div class="valid-feedback">
                     صحيح
            </div>

            <div class="invalid-feedback">
                <span>يجب إدخال الراتب بشكل صحيح</span>
            </div>
        </div>
        <div class="form-check ">
...
import VueUploadMultipleImage from 'vue-upload-multiple-image'
  ...
  export default {
    ...
    data() {
        return {
            loading : false,
            titlePage:'تعديل معلومات الموظف',
            record:{
                name:'',
                email:'',
                phone:0,
    ...
    methods: {
        async onSubmit() {
            this.loading=true;
            let response = await this.Api('PUT',`employees/${this.record.id}`,this.record);
            this.loading=false;
    ...
    async mounted(){
        ...
        let response = await this.Api('GET',`employees/${this.$route.params.id}`);
