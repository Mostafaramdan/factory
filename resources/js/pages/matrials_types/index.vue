...
export default {
    ...
    methods:{
        show(index){
             this.$router.push( {name:'matrials_typesShow' , params: { id: this.records[index].id }});
        },
        create(){
             this.$router.push( {name:'matrials_typesCreate'});
        },
        update(index){
             this.$router.push( {name:'matrials_typesUpdate', params: { id: this.records[index].id }});
        },
        ...
        async getRecords(){
            ...
            let response = await this.Api('GET','matrials_types',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`matrials_types/${this.records[index].id}`);
    ...
    metaInfo() {
        return {
            title: `${this.$store.state.appName} -   مواد خام  `,
        }
    },
    watch :{
        search:function(val){
