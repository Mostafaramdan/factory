...
<script>
  ...
  export default {
    ...
    data() {
        return {
            loading : false,
            titlePage:'تعديل  التكاليف المتغيرة',
            record:{
                name:'',
    ...
    methods: {
        async onSubmit() {
            this.loading=true;
            let response = await this.Api('PUT',`variable_costs/${this.record.id}`,this.record);
            this.loading=false;
    ...
    async mounted(){
        ...
        let response = await this.Api('GET',`variable_costs/${this.$route.params.id}`);
