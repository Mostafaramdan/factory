...
export default {
    ...
    methods:{
        show(index){
             this.$router.push( {name:'variable_costsShow' , params: { id: this.records[index].id }});
        },
        create(){
             this.$router.push( {name:'variable_costsCreate'});
        },
        update(index){
             this.$router.push( {name:'variable_costsUpdate', params: { id: this.records[index].id }});
        },
        ...
        async getRecords(){
            ...
            let response = await this.Api('GET','variable_costs',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`variable_costs/${this.records[index].id}`);
    ...
    metaInfo() {
        return {
            title: `${this.$store.state.appName} -   التكاليف المتغيرة  `,
        }
    },
    watch :{
        search:function(val){
