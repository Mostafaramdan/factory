                        ...
                        <table class="table  table-striped table-dark table-bordered table-hover ">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">الاجمالي ({{totalvariable_costs}})</th>
                                    <th></th>
                                </tr>
                            </thead>
                            <tbody>
                        ...
                        <table class="table  table-striped table-dark table-bordered table-hover ">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">الاجمالي ({{totalproduction_materials}})</th>
                                    <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody>
...
<script>
  ...
  export default {
    ...
    data() {
        return {
            titlePage:'  إضافة إنتاج جديد  ',
            loading : false,
            production_materials:[],
            new_production_materials:'',
            production_materialsPrice:'',
            production_materialsQuantity:'',
            variable_costs:[],
            new_variable_costs:'',
            variable_costsTotal:'',
            record:{
    ...
    methods: {
        async onSubmit() {
            this.loading=true;
            let response = await this.Api('POST','productions',this.record);
            this.loading=false;
    ...
    computed: {
        allValidation(){
            return  this.record.variable_costs.length > 0 && this.record.production_materials.length > 0 && !this.loading
        },
        totalvariable_costs(){
            var total = 0;
    ...
    async mounted(){
        ...
        let response = await this.Api('GET',`getProduction_materials`);
        this.variable_costs = response.data.variable_costs;
        this.production_materials= response.data.matrials_types;
    },
    metaInfo() {
        return {
