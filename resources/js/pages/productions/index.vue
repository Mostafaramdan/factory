            ...
            <table class="table table-striped table-dark table-bordered table-hover  mb-2"  >
                ...
                <tbody>
                    <tr v-for="(record,index) in records" :key="index">
                        ...
                        <td>
                            <span v-if="record.quantity">{{( (record.total + record.salaries)/record.quantity).toFixed(3)}}</span>
                            <span v-else>لم يحدد بعد</span>
                        </td>
...
export default {
    ...
    methods:{
        show(index){
             this.$router.push( {name:'productionsShow' , params: { id: this.records[index].id }});
        },
        create(){
             this.$router.push( {name:'productionsCreate'});
        },
        update(index){
             this.$router.push( {name:'productionsUpdate', params: { id: this.records[index].id }});
        },
        ...
        async getRecords(){
            ...
            let response = await this.Api('GET','productions',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`productions/${this.records[index].id}`);
    ...
    metaInfo() {
        return {
            title: `${this.$store.state.appName} -   الإنتاج   `,
        }
    },
    watch :{
        search:function(val){
